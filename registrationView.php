...
$clinic_caption = array("病院ID", "パスワード", "病院名", "病院名（読み）",
		       ...
		       "電話番号");
$clinic_vars_min = array(1, 8, 1, 1, 1, 8, 1, 1, 12);
$clinic_vars_max = array(10, 20, 50, 100, 50, 8, 255, 255, 13);
...
for ($cnt = 0; $cnt < count($clinic_attribute); $cnt++) {
  ...
  if(isset($_POST[$clinic_attribute[$cnt]])){
    $clinic_var[$cnt] = $_POST[$clinic_attribute[$cnt]];
  }
}
//print_r($clinic_var);
...
if ($_POST["reset"]) {
  echo "reset"."<P>";
  for ($cnt = 0; $cnt < count($clinic_var); $cnt++) {
    $clinic_var[$cnt] = null;
  }
}
//var_dump($_POST);
...
if ($_POST["submit"] ||$_POST["verify"] ) {
    ...
    if($cmd == "add"){
      if ($_POST["verify"]) {
	$table_error = checkInput($clinic_var, $clinic_vars_min, $clinic_vars_max, $table_error);
	$verify = true;
	...
	if(userIdVerify($clinic_var[0])){
	  if(passwordVerify($clinic_var[0], $clinic_var[1])){
	...
	for ($cnt = 1; $cnt < count($clinic_attribute); $cnt++) {
	  if($cnt == $last_item){
	    $str .= $clinic_attribute[$cnt] . "='" . $clinic_var[$cnt] . "'";
	  } else {
	    $str .= $clinic_attribute[$cnt] . "='" . $clinic_var[$cnt] . "', ";
	  } 
	}
	$str .= " WHERE " . $clinic_attribute[0] . "=" . $clinic_var[0] . ";";
	...
	if($verify == true){
	  ...
	  $clinic_var = getClinicData($clinic_var[0], $clinic_var[1], $clinic_attribute);
	  
	}
      }
      if ($_POST["submit"]) {
	$verify = true;
	$str = "DELETE FROM clinic WHERE " . $clinic_attribute[0] . " = '" . $clinic_var[0] . "' AND "  . $clinic_attribute[1] . " = '" . $clinic_var[1] . "'";
	$result = mysql_query($str);
...
for ($nc = 0; $nc < count($clinic_attribute); $nc++) {
  ...
  if($verify == false){  
    ...
    $str = $err . "<input type='text' name='" . $clinic_attribute[$nc] . "' value='" . $clinic_var[$nc] . "' size=50 />";
  } else if($verify == true && $_POST["verify"]){
    // クエリー実行
    $str = htmlspecialchars($clinic_var[$nc], ENT_QUOTES, "UTF-8") ."<input type='hidden' name='" . $clinic_attribute[$nc] . "' value='" . $clinic_var[$nc] . "' />";
  } else {
    $str = htmlspecialchars($clinic_var[$nc], ENT_QUOTES, "UTF-8");
  }
