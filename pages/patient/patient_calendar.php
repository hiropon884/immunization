 ...
 */
$db = $smarty->getDb();
$tableItem = $db->getClinicTermSetting($clinic_id);
/*
  //// クエリーの実行
...
foreach ($tableItem as $row) {
    $table->setCellContents($nc + 1, 0, $row['immunization_name']);
    $table->setCellContents($nc + 1, 1, $row['regular']);
    $table->setCellContents($nc + 1, 2, $row['times']);
    $baseSec = mktime(0, 0, 0, $month + $row['term_start'], $day, $year);
    $table->setCellContents($nc + 1, 3, date("Y-m-d", $baseSec));
    $str = "<button type=\"submit\" name=\"medicine_params\" value=\"" .
            $row['immunization_id'] . "_" . $row['times'] . "\">予約</button>";
