...
SQL;
    ...
    public function verifyBookData($input, $type) {
        $sql = <<<SQL
SELECT *
FROM book 
WHERE person_id = :person_id
...
/*
    ...
    public function insertBookData($input) {
        $sql = <<<SQL
INSERT INTO book 
VALUES (:person_id, :immunization_id, :number, 
        :day, :lot_num, :state)
SQL;
        ...
        $this->bindValueWithType($sth, ':immunization_id', $input[1]);
        $this->bindValueWithType($sth, ':number', $input[2]);
        $this->bindValueWithType($sth, ':day', $input[3]);
        $this->bindValueWithType($sth, ':lot_num', $input[4]);
        $this->bindValueWithType($sth, ':state', $input[5]);
    ...
    public function updateBookData($input) {
        $sql = <<<SQL
UPDATE book 
SET person_id = :person_id, immunization_id = :immunization_id,
number = :number, day = :day, lot_num = :lot_num, state = :state
WHERE person_id = :person_id
AND immunization_id = :immunization_id
AND number = :number
SQL;
    ...
    public function getBookData($input) {
        $sql = <<<SQL
SELECT *
FROM book 
WHERE person_id = :person_id
...
SQL;
    ...
    public function deleteBookData($input) {
        $sql = <<<SQL
DELETE
FROM book 
WHERE person_id = :person_id
...
WHERE book.person_id = :person_id 
AND book.state = :state
SQL;

        $sth = $this->prepare($sql);
        $this->bindValueWithType($sth, ':person_id', $id);
        $this->bindValueWithType($sth, ':state', $state);
        $rows = $this->prepared_query($sth);

        return $rows;
    }

    public function getClinicTermSetting($clinic_id) {
        $sql = <<<SQL
SELECT body.immunization_id, body.immunization_name, 
       body.regular, body.kinds, term.times, term.term_start 
FROM immunization as body 
INNER JOIN immunization_term as term 
ON body.immunization_id = term.immunization_id 
WHERE term.clinic_id = :clinic_id
AND term.is_enable = 1
SQL;
